...
public class CategoriaServicoControllerTest
{
    ...
    public async Task Deve_Retornar_Lista_CategoriaServico()
    {
        ...
        _repositoryMock.Setup(s => s.GetAll()).ReturnsAsync(categoriasFake);
        //Act
    ...
    {
        return new CategoriaServico() { Desc = "Descricao teste", Titulo = "Titulo teste", DataCadastro = DateTime.Now, Id = Id.HasValue ? Id.Value : Guid.Empty, UsrCadastro = Guid.NewGuid() };
    }

    private static ICollection<CategoriaServico> RetornarListaCategoriasFake()
    {
        return new List<CategoriaServico>() { new CategoriaServico() { Titulo = "Titulo Categoria Fake", Desc = "Descricao Categoria Fake" } };
    }
}
